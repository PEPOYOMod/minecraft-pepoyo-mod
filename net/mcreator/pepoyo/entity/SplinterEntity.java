...
import net.mcreator.pepoyo.procedures.SplinterDanSheWuJiZhongWanJiaShiProcedure;
...
public class SplinterEntity extends AbstractArrow implements ItemSupplier {
   public static final ItemStack PROJECTILE_ITEM;

   public SplinterEntity(SpawnEntity packet, Level world) {
      super((EntityType)PepoyoModEntities.SPLINTER.get(), world);
   }

   public SplinterEntity(EntityType<? extends SplinterEntity> type, Level world) {
      super(type, world);
   }

   public SplinterEntity(EntityType<? extends SplinterEntity> type, double x, double y, double z, Level world) {
      super(type, x, y, z, world);
   }

   public SplinterEntity(EntityType<? extends SplinterEntity> type, LivingEntity entity, Level world) {
   ...
   public void m_6123_(Player entity) {
      ...
      SplinterDanSheWuJiZhongWanJiaShiProcedure.execute(entity);
   }
   ...
   public static SplinterEntity shoot(Level world, LivingEntity entity, RandomSource source) {
      return shoot(world, entity, source, 1.0F, 5.0D, 5);
   }

   public static SplinterEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
      return shoot(world, entity, source, pullingPower * 1.0F, 5.0D, 5);
   }

   public static SplinterEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
      SplinterEntity entityarrow = new SplinterEntity((EntityType)PepoyoModEntities.SPLINTER.get(), entity, world);
   ...
   public static SplinterEntity shoot(LivingEntity entity, LivingEntity target) {
      SplinterEntity entityarrow = new SplinterEntity((EntityType)PepoyoModEntities.SPLINTER.get(), entity, entity.m_9236_());
